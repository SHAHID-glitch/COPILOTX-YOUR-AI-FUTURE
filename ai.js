...
router.post('/text-to-speech', optionalAuth, async (req, res) => {
    try {
        ...
        // Determine python path (cross-platform: Windows and Linux/HuggingFace)
        console.log('🔍 Checking TTS paths...');
        console.log('Platform:', process.platform);

        const isWindows = process.platform === 'win32';
        // Allow override via env var if provided
        let pythonCommand = process.env.EDGE_TTS_PYTHON || process.env.PYTHON_BIN || 'python3';

        if (!process.env.EDGE_TTS_PYTHON && !process.env.PYTHON_BIN) {
            if (isWindows) {
                // Windows: try local venv first, then system python
                const venvWin = path.join(__dirname, '..', '.venv', 'Scripts', 'python.exe');
                if (fs.existsSync(venvWin)) {
                    pythonCommand = `"${venvWin}"`;
                } else {
                    pythonCommand = 'python';
                }
            } else {
                // Linux/HF Spaces: try /opt/venv (Dockerfile), then local .venv, then system python3
                const hfVenv = '/opt/venv/bin/python';
                const localVenv = path.join(__dirname, '..', '.venv', 'bin', 'python');

                if (fs.existsSync(hfVenv)) {
                    pythonCommand = hfVenv;
                } else if (fs.existsSync(localVenv)) {
                    pythonCommand = localVenv;
                } else {
                    // As a last check, if python3 is not resolvable, try 'python'
        ...
        // Quick sanity check: verify edge_tts import works to provide clearer errors
        try {
            const checkCmd = `${pythonCommand} -c "import edge_tts; print('ok')"`;
            await execPromise(checkCmd, { timeout: 15000 });
        } catch (pyCheckErr) {
            const instructions = isWindows
                ? 'Make sure .venv/Script/python.exe exists and install edge-tts in that venv: pip install edge-tts'
                : 'Ensure Python is available (python3) and edge-tts is installed. In Docker, we use /opt/venv; otherwise add apt.txt (python3, python3-pip) and requirements.txt (edge-tts) in Hugging Face Space.';
        ...
        // Generate audio using Edge TTS (works on Windows and Linux)
        const ttsCommand = `${pythonCommand} -m edge_tts --text "${escapedText}" --write-media "${outputPath}" --voice "${selectedVoice}"`;
        ...
        let tip = 'On Hugging Face Spaces, add apt.txt with python3 and python3-pip, and requirements.txt with edge-tts.';
        
        if (error.message.includes('edge_tts')) {
            errorMessage = 'Edge-TTS module not found';
            tip = 'Install dependencies: pip install -r requirements.txt or pip install edge-tts';
        } else if (error.message.includes('python') || error.message.includes('ENOENT')) {
            errorMessage = 'Python not found or edge-tts not installed';
            tip = 'For Hugging Face Spaces: include apt.txt with python3 and python3-pip, and requirements.txt with edge-tts. For local: install Python 3 and run pip install -r requirements.txt';
        }
