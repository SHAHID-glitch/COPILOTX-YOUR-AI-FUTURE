console.log('📝 Copilot script starting...');
        ...
        if (typeof API_BASE_URL === 'undefined') {
            ...
            const isHuggingFaceEmbed = window.location.hostname === 'huggingface.co';
            var API_BASE_URL = isLocalhost
                ? `http://${window.location.hostname}:${window.location.port || CONFIG.DEFAULT_PORT}`
                : isHuggingFaceEmbed
                    ? 'https://shahidmalik786-copilotx-ai.hf.space'
    ...
    <div style="margin-top: 12px; padding: 8px; background: #e8f3ff; border-radius: 6px; font-size: 12px; color: #0c2d6b;">
        ...
        async function handlePdfUpload(file) {
            ...
            try {
                ...
                if (!response.ok) {
                    const errPayload = await safeParseJson(response);
                    const msg = errPayload?.error || `Upload failed with status ${response.status}`;
                    throw new Error(msg);
                }
...
${ocr.extractedText.substring(0, 500)}${ocr.extractedText.length > 500 ? '\n...[truncated]' : ''}`;
        ...
        async function generatePodcastFromChat(topic) {
            ...
            try {
                ...
                for (let i = 0; i < chunks.length; i++) {
                    if (chunks.length > 1) {
                        loadingMsg.innerHTML = formatTextToHtml(`🎙️ Script generated! Converting to speech (${i + 1}/${chunks.length})...`);
                    }
                    ...
                    const part = await ttsResponse.blob();
                    audioBlobs.push(part);
                }

                // Merge audio parts
                const audioBlob = audioBlobs.length === 1 ? audioBlobs[0] : new Blob(audioBlobs, { type: 'audio/mpeg' });
                ...
                loadingMsg.innerHTML = formatTextToHtml(`❌ Error: ${error.message || 'Failed to generate podcast'}`);
            }
        }
        ...
        async function generatePodcast() {
            ...
            try {
                ...
                console.log(`📝 Processing ${textChunks.length} audio chunk(s)...`);
                ...
                for (let i = 0; i < textChunks.length; i++) {
                    if (textChunks.length > 1) {
                        document.getElementById('podcastGeneratingText').textContent = 
                            `Converting to speech (${i + 1}/${textChunks.length})...`;
                    }
                ...
                if (audioBlobs.length === 1) {
                    ...
                    // Combine multiple blobs by concatenating them
                    finalAudioBlob = new Blob(audioBlobs, { type: 'audio/mpeg' });
                    console.log(`✅ Combined ${audioBlobs.length} audio chunks`);
                }
